...
(function (g) {
  ...
  /**
   * Hide listings that use Facebook's common placeholder prices with "make offer" noise.
   */
  function isPlaceholderMakeOffer(L) {
  ...
  /**
   * Heuristic deal quality (on-device). Replace/extend with your own API in researchDeal().
   */
  function scoreDeal(L) {
    ...
    var hours =
      typeof L.listedAgeHours === "number" && isFinite(L.listedAgeHours)
        ? L.listedAgeHours
        : null;
    if (hours != null) {
      if (hours <= 6) {
  ...
  /**
   * Hook for richer checks (price comps, LLM, etc.). Must return a Promise resolving to
   * { score: number, label: string, reasons: string[] }. Default uses scoreDeal().
   * Override from any page script before dashboard.js: window.shelfResearchDeal = async (L) => ({ ... })
   */
  async function researchDeal(L) {
    if (typeof g.shelfResearchDeal === "function") {
      try {
        return await g.shelfResearchDeal(L);
      } catch {
        /* fall through */
      }
    }
